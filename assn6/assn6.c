...
/*void LIST_MERGE_LEFT(List** rlist,int l,int u,List** nlist)
{
	List *temp,*tmp;
	if(rlist==NULL)
		return nlist;
	temp = rlist;
	while(temp->next!=NULL || temp->next->data>=l)
	{
		temp=temp->next;
	}
	if(nlist==NULL)
...
/*void LIST_MERGE_RIGHT(List** rlist,int l,int u,List** nlist)
{
	List *temp,*tmp;
	if(rlist==NULL)
		return nlist;
	temp=rlist;
	while(temp->next!=NULL || temp->next->data>u)
		{temp=temp->next;}
	if()
}*/
...
// 			//parent = root;
// 			printf("1>>>>>>> %d %d \n",root->l,root->u);
// 			LIST_MERGE_MID(root->list,l,u,node->list);
// 			node->l=min(root->l,node->l);
// 			node->u=max(root->u,node->u);
// 			root->l=0;
// 			root->u=0;
...
// 			printf("2>>>>>>> %d %d \n",root->l,root->u);
// 			printf("******%d %d \n",root->u,node->u);
// 			LIST_MERGE_LEFT(root->list,l,u,node->list);
// 			node->l=l;
...
// 			printf("3>>>>>>> %d %d \n",root->l,root->u);
// 			printf("******%d %d \n",root->u,node->u);
// 			LIST_MERGE_RIGHT(root->list,l,u,node->list);
